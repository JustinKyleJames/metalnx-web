...
public class ResourceController {
	...
	public String findResourcesByName(Model model, @PathVariable String resourceName)
			throws DataGridConnectionRefusedException {
	...
	public String showAddResourceFormWithParentSet(Model model, @PathVariable String parent)
			throws DataGridConnectionRefusedException {
	...
	public String addResource(@ModelAttribute ResourceForm resourceForm, HttpServletRequest httpServletRequest,
		...
		// if a resource A was created in the data grid, now we are adding this resource
		// A as a
	...
	public String showModifyResourceForm(Model model, @PathVariable String resourceName)
			throws DataGridConnectionRefusedException {
		if (dataGridResources == null || dataGridResources.isEmpty()) {
			dataGridResources = resourceService.findAll();
		}
		...
		List<DataGridResourceType> coordinatingResources = getCoordinatingResources(
				resourceService.listResourceTypes());
	 ...
	 *             if resource map json cannot be created
	 * @throws DataGridConnectionRefusedException
	...
	public String getResourceTreeStructureFordashboard(Model model)
			throws JSONException, DataGridConnectionRefusedException {
	...
	private DataGridResource getDataGridResource(ResourceForm resourceForm) {
		...
		if (resourceForm.getType().compareToIgnoreCase(DataGridServerType.ISILON.name()) == 0) {
			...
			// system
			String context = String.format(ISILON_CONTEXT_STRING, resourceForm.getIsiHost(), resourceForm.getIsiPort(),
					resourceForm.getIsiUser());
	...
	/**
	 * Gets from a list of {@link DataGridResourceType} all resources types whose
	 * classified as coordinating resources.
	 *
	...
	/**
	 *
	 * Gets from a list of {@link DataGridResourceType} all resources type whose
	 * classified as storage resources.
	 *
	 * @param dataGridResourceTypes
	 *            list of all resource types available in the data grid
	...
	private void getResourcesMap(Model model, boolean isDashboard)
			throws JSONException, DataGridConnectionRefusedException {
